# Areyhya — SOC Analyst Portfolio

## About Me
Entry-level cybersecurity analyst with cybersecurity coursework from WGU and the Google Cybersecurity Certificate.  
Target roles: SOC Analyst I / Security Operations Analyst.  
I enjoy log analysis, incident investigation, and mapping activity to MITRE ATT&CK.## Skills Demonstrated

## Skills
- Threat Intelligence Analysis
- Security Investigations
- OSINT Research
- DNS Analysis
- Incident Documentation
- Indicators of Compromise (IOC) Identification
- **Security Analysis**
- Incident Investigation
- Threat Detection

**Frameworks & Methodologies**
- MITRE ATT&CK
- Incident Response Workflow

**Tools**
- TryHackMe
- Linux
- GitHub
  
## Cybersecurity Investigation Projects

## Investigation Projects

Below are hands-on cybersecurity investigations demonstrating SOC analyst workflows, threat intelligence analysis, and security research.

### Threat Intelligence Investigation

Domain analysis using multiple threat intelligence platforms including VirusTotal, WHOIS, and DNS lookup to identify potential indicators of compromise.

Tools Used:
- VirusTotal
- AbuseIPDB
- WHOIS
- DNS Analysis

[View Investigation Project](https://github.com/areyhya/threat-intelligence-investigation)
---

## Portfolio Overview

| Investigation | Description |
|---|---|
| [01 - Brute Force Login Investigation](./01-Brute-Force-Investigation) | Analyzed SSH authentication failures to identify brute force behavior; documented IoCs and response actions |
| 02 - Phishing Investigation *(coming soon)* | Email header + link analysis with recommended SOC response |
| -03 [View Threat Intelligence Investigation](https://github.com/areyhya/threat-intelligence-investigation) | Analysis of a suspicious domain using OSINT tools such as VirusTotal, WHOIS, and DNS lookup to identify indicators of compromise) |

---

### Brute Force Login Investigation
Investigated repeated SSH authentication failures to identify a potential brute force attack. Analyzed authentication logs, identified indicators of compromise, and mapped activity to MITRE ATT&CK.

Project folder: 01-Brute-Force-Investigation
## Tools

- TryHackMe
- Linux
- GitHub
- Basic Log Analysis
- Security Monitoring Concepts

**Technical Knowledge**
- Networking Fundamentals
- Authentication Logs
- Security Monitoring

  
## Currently Learning

- SOC investigation techniques
- Log analysis and threat detection
- Security monitoring workflows
